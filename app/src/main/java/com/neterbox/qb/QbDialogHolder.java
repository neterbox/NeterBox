...
public class QbDialogHolder {
    ...
    public boolean hasPrivateDialogWithUser(QBUser user){
        return getPrivateDialogWithUser(user, null) != null;
    }

    public QBChatDialog getPrivateDialogWithUser(QBUser user, String project_name){
        Log.e("dialog_map", String.valueOf(dialogsMap.values()));
        for (QBChatDialog chatDialog : dialogsMap.values()){

            Log.e("get_type", String.valueOf(chatDialog.getType()));
            Log.e("get_id", String.valueOf(chatDialog.getOccupants().contains(user.getId())));

            if (QBDialogType.PRIVATE.equals(chatDialog.getType()) && chatDialog.getOccupants().contains(user.getId())){
                Log.e("with_user", String.valueOf(chatDialog));
                return chatDialog;
            }
            else if(QBDialogType.GROUP.equals(chatDialog.getType())){

                chatDialog.setName(project_name);
//                chatDialog.setPhoto(project_pic);
            }
        }
